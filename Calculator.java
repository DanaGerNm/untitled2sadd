...
class Romes extends Number {
    ...
    private String convert_result_to_Romes(int n) {
        ...
        if (remains != 0) {
            try {
                return convert_result_to_Romes(n - remains) + roman_letters_9[remains - 1];
            } catch (ArrayIndexOutOfBoundsException e) {
                sign = "-";
                return convert_result_to_Romes(n - remains) + roman_letters_9[(remains + 1) * -1];
            }
        }
        return null;
    }
    ...
    private int convert_to_int(String romes_value){
        ...
        for (int i = 0; i < value_char.length; i++) {
            switch (value_char[i]) {
                ...
                default:
                    System.out.println("Не верный ввод римских чисел:" + "\n" +
                            "Пример обозначения: I = 1" + "\n" +
                                                "V = 5" + "\n" +
                                                "X = 10");
                    break;
            }
...
class Arabic extends Number {
    ...
    public void div() {
        try{
            ...
            System.out.print("Аккуратно,деление на 0. ");
            return;
        }
